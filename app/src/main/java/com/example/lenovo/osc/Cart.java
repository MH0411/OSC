...
public class Cart implements Serializable {
    ...
    public void add(final Saleable sellable, int quantity) {
        ...
        totalPrice = totalPrice.add(BigDecimal.valueOf(sellable.getPrice()).multiply(BigDecimal.valueOf(quantity)));
    ...
    public void update(final Saleable sellable, int quantity) throws ProductNotFoundException, QuantityOutOfRangeException {
        ...
        BigDecimal productPrice = BigDecimal.valueOf(sellable.getPrice()).multiply(BigDecimal.valueOf(productQuantity));

        cartItemMap.put(sellable, quantity);

        totalQuantity = totalQuantity - productQuantity + quantity;
        totalPrice = totalPrice.subtract(productPrice).add(BigDecimal.valueOf(sellable.getPrice()).multiply(BigDecimal.valueOf(quantity)));
    }
    ...
    public void remove(final Saleable sellable, int quantity) throws ProductNotFoundException, QuantityOutOfRangeException {
        ...
        totalPrice = totalPrice.subtract(BigDecimal.valueOf(sellable.getPrice()).multiply(BigDecimal.valueOf(quantity)));
        totalQuantity -= quantity;
    }
